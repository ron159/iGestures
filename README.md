...
打开 URL、启动应用或运行其他动作。手势轨迹只在本机内存中处理。
...
- 🗂️ **应用管理**：按全局、应用分组和单个应用管理手势；匹配优先级为
  `单个应用 → 应用分组 → 全局`。
- ⚡ **丰富动作**：支持键盘快捷键、URL、文件与文件夹、应用菜单、固定文本、
  窗口管理、系统动作、Apple 快捷指令、动作序列和经确认的受控脚本。
- 🔎 **快捷动作库**：按窗口、浏览器、访达、桌面、编辑、媒体和自动化分类搜索，
  支持收藏、最近使用和保存自定义预设，主动作与第二触发动作共用。
- 📚 **脚本库**：保存常用 AppleScript 和 Shell 脚本，并从内置系统、访达和效率
  脚本中一键套用到手势。
- 🎯 **自然触发**：按住时长或移动距离任一达到阈值即可开始识别，并支持
  Repeat Mode、触控板修饰键手势和三档识别灵敏度。
- 🎨 **即时反馈**：支持多显示器轨迹覆盖层、识别结果提示和可选触觉反馈。
- 🛡️ **可靠与私密**：映射采用版本化 JSON、原子写入和损坏恢复；不记录轨迹、
  键入文本或窗口内容。
- 🔐 **系统集成**：分项申请辅助功能、输入监听和事件发送权限，支持登录时启动。
- 🔄 **版本更新**：自动检查 GitHub 最新正式 Release，并提供更新页面入口。
...
社区版本使用本地临时签名，未经过 Apple 公证。首次启动如被阻止，请在
“系统设置 → 隐私与安全性”中选择“仍要打开”，然后在 iGestures 权限页按提示
授予辅助功能、输入监听和事件发送权限。
